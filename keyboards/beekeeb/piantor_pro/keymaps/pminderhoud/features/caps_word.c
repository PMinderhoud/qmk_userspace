...
// Based on https://github.com/Ga68/qmk_firmware/blob/my_layout/keyboards/sofle/keymaps/Ga68/caps_word.c
...
uint16_t g_caps_word_letter_count = 0;
            ...
            switch (keycode) {
                ...
                case KC_BACKSPACE:
                    if (keycode == KC_BACKSPACE) {
                        g_caps_word_letter_count -= 1;
                        // Apply oneshot shift when the first letter was removed when in Pascal Case
                        if (g_caps_word_letter_count <= 0 && g_caps_word_mode == CWMODE_PASCAL_CASE) {
                            add_weak_mods(MOD_BIT(KC_LSFT));  // Apply shift to next key.
                            g_caps_word_letter_count = 0;
                        }
                    }
                    else {
                        g_caps_word_letter_count += 1;
...
void caps_word_set_user(bool active) {
    if (active) {
        ...
        // Go back to make sure that when it turns on next without any specification (ex. through
...
bool toggle_caps_word_mode(caps_word_mode_t new_mode) {
    ...
    if (caps_word_was_off || prior_mode != new_mode) {
        ...
        g_caps_word_letter_count = 0;
        if (new_mode == CWMODE_PASCAL_CASE) {
            add_oneshot_mods(MOD_LSFT);
        }
    }
